...
for i in admin-log4j.xml httpd-log4j.xml ftpd-log4j.xml sshd-log4j.xml as2d-log4j.xml pesitd-log4j.xml socks-log4j.xml tools-log4j.xml; do
   ...
   if [[ -f $LOG4J_FILE ]];then
      ...
      sed -i 's/<AppenderRef ref="ServerLog"\/>/<!--<AppenderRef ref="ServerLog"\/>-->/g' $LOG4J_FILE
      sed -i 's/<AppenderRef ref="AuditLogAppender"\/>/<!--<AppenderRef ref="AuditLogAppender"\/>-->/g' $LOG4J_FILE
      echo "Finished"
   fi
